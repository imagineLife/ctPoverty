...
d3.queue()
  ...
  .defer(d3.csv, "data.csv", function(d) { 
    if (isNaN(d.percent)) {
        ...
        incomeData.set(d.id, +d.percent)
    }
    switch(true){
        case (d.percent < (lvl.one - .1)):
            colorRatio["level1"]++;
            break;
        case (d.percent >= lvl.one && d.percent< (lvl.two - .1)):
            colorRatio["level2"]++;
            break;
        case (d.percent >= lvl.two && d.percent < (lvl.three - .1)):
            colorRatio["level3"]++;
            break;
        case (d.percent >= lvl.three && d.percent < (lvl.four - .1)):
            colorRatio["level4"]++;
            break;
        default:
            colorRatio["level5"]++;
...
function ready(error, data) {
    ...
    */
    ...
    //       'y' : d => barYScale(d.percent),
    //       'width' : d => barXScale.bandwidth(),
    //       'height' : d => heightLessMargins - barYScale(d.percent),
    //       'fill' : d => greenColorScale(d.percent),
    //       'class':'barClass'
    //     });
    ...
    //   .text(function (d) {
    //     let f = d3.format(".2s");
    //     return (d.percent == "250001") 
    //         ? `$250K+` 
    //         :`≈ $${f(d.percent)}`; 
    //   })
    //   .attrs({
    //     "x": d => ( barXScale(d.town) + barXScale.bandwidth() ),
    //     "y": function (d) { return barYScale(d.percent)},
    //     // "text-anchor": "middle",
    //     "class":"barText"
    //   })
    //   .style("fill", "white");
